...
import { Trash2, Search, PackagePlus, X, Tag, Plus } from "lucide-react"
...
export function AddPackageDialog({ items, masterCategories = [], masterLabels = [] }: AddPackageDialogProps) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    setErrorMsg(null)
    if (cart.length === 0) {
      setErrorMsg("Pilih minimal 1 aset untuk paket rental.")
      return
    }
    if (!packageName.trim()) {
  ...
  return (
    <>
        ...
        <DialogContent aria-describedby={undefined} className="bg-zinc-950 border-zinc-800 text-zinc-50 flex flex-col max-h-[90vh] overflow-hidden">
          ...
          <form action={handleSubmit} className="flex flex-col gap-4 pt-2 flex-1 overflow-hidden">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 shrink-0">
              {/* Kolom Kiri: Nama Paket & Deskripsi */}
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <Label htmlFor="kategoriUtama">Kategori Utama</Label>
                  <select
                    id="kategoriUtama"
                    name="kategoriUtama"
                    value={kategoriUtama}
                  ...
                  {filteredItems.length > 0 ? (
                    filteredItems.map(item => (
                      <div key={item.id} className="p-2.5 hover:bg-zinc-700 cursor-pointer border-b border-zinc-700/50 flex justify-between items-center gap-3" onClick={() => addToCart(item)}>
                        ...
                        <p className="text-xs font-semibold text-zinc-400 shrink-0">Stok: {item.quantity}</p>
                      </div>
                    ))
            ...
            <div className="flex flex-col gap-2 flex-1 min-h-0 bg-zinc-900/30 p-3 rounded-xl border border-zinc-800">
              <Label className="text-xs text-zinc-400">Daftar Isi Paket ({cart.length} item dipilih)</Label>
              {cart.length === 0 ? (
                <div className="flex-1 flex items-center justify-center p-6 border border-dashed border-zinc-800 rounded-lg text-center text-zinc-500 text-sm">Belum ada aset ditambahkan ke paket.</div>
              ) : (
      ...
      <DemoRestrictionDialog
        ...
        message="Akun yang sedang digunakan adalah akun demo dan bersifat read-only. Pembuatan template paket rental dinonaktifkan pada akun ini."
      />
    </>
  )
}