  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { setOpen(isOpen); if (!isOpen) { setErrorMsg(null); setCart([]); }}}>
      <DialogTrigger asChild>
        <Button className="bg-emerald-950/50 border border-emerald-900 text-emerald-400 hover:bg-emerald-900 hover:text-emerald-50">
      ...
      <DialogContent className="bg-zinc-950 border-zinc-800 text-zinc-50 overflow-visible max-h-[90vh] overflow-y-auto">
        ...
        <form action={handleSubmit} className="space-y-4 pt-2">
          ...
          <div className="space-y-2 pt-2">
            <Label>Daftar Barang Disewa</Label>
            {cart.length === 0 ? (
              <div className="p-6 border border-dashed border-zinc-800 rounded-lg text-center text-zinc-500 text-sm">Belum ada barang yang dipilih.</div>
            ) : (
          ...
          <Button type="submit" disabled={cart.length === 0} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white">Proses Sewa (Rental)</Button>
        </form>
      </DialogContent>
    </Dialog>
  )
