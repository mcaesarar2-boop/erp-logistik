  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { setOpen(isOpen); if (!isOpen) { setErrorMsg(null); setCart([]); }}}>
      <DialogTrigger asChild>
        <Button className="bg-red-950/50 border border-red-900 text-red-400 hover:bg-red-900 hover:text-red-50">
      ...
      <DialogContent className="bg-zinc-950 border-zinc-800 text-zinc-50 overflow-visible max-h-[90vh] overflow-y-auto">
        ...
        <form action={handleSubmit} className="space-y-4 pt-2">
          ...
          <div className="space-y-2 pt-2">
            <Label>Daftar Aset untuk Pemeliharaan</Label>
            {cart.length === 0 ? (
              <div className="p-6 border border-dashed border-zinc-800 rounded-lg text-center text-zinc-500 text-sm">Belum ada aset yang dipilih.</div>
            ) : (
                ...
                {cart.map(item => (
                  <div key={item.id} className="bg-zinc-900 p-3 rounded-lg border border-zinc-800 flex justify-between items-center gap-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input type="number" min="1" max={item.quantity} value={item.qty} onChange={(e) => updateQty(item.id, parseInt(e.target.value)||1)} className="w-16 h-8 text-xs bg-zinc-950 border-zinc-700 text-center" />
                      <button type="button" onClick={() => setCart(cart.filter(c => c.id !== item.id))} className="text-zinc-500 hover:text-red-400"><Trash2 className="w-4 h-4"/></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Button type="submit" disabled={cart.length === 0} className="w-full bg-red-600 hover:bg-red-700 text-white">Proses Pemeliharaan</Button>
        </form>
      </DialogContent>
    </Dialog>
  )
